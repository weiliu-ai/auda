...
class Evaluator(object):
    ...
    def average_recall(self):
        recalls = np.random.rand(self.num_classes)
        for i in range(self.num_classes):
            recalls[i] = self.confusion_matrix[i, i] / np.sum(self.confusion_matrix[i])
    ...
    def average_precision(self):
        ...
        for i in range(self.num_classes):
            precisions[i] = self.confusion_matrix[i, i] / np.sum(self.confusion_matrix[:, i])
