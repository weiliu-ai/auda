    ...
    with torch.no_grad():
        ...
        for iteration, data_batch in enumerate(dataloader):
            ...
            for batch_ind in range(len(seg_label)):
                ...
                weights_class = np.array( [1.89090012, 2.0585112, 3.1970535, 3.1111633, 1., 2.93751704, 1.92053733,
                                           1.47886874, 1.04654198, 1.78266561], float)
                ...
                if id_batch % 100 == 99 and batch_ind == 0:
                    img_indices = data_batch['img_indices'][batch_ind]
                    img_title = 'results/SemanticKITTI/image_{}_{}'.format(id_batch, batch_ind)
                    image_name = img_title + '.jpg'
                    new_img_PIL.save(image_name, quality=95)
                    # draw ground truth labels and the proposed scheme
                    # draw_points_image_labels(bg, img_indices, curr_seg_label, color_palette_type='SemanticKITTI',
                    #                         point_size=1, figure_title=img_title + '_gt.jpg')
                    # draw_points_image_labels(bg, img_indices, pred_label_ensemble, color_palette_type='SemanticKITTI',
                    #                          point_size=1, figure_title=img_title + '_auda.jpg')
                    # draw_points_image_labels(bg, img_indices, pred_label_ensemble_tm, color_palette_type='SemanticKITTI',
